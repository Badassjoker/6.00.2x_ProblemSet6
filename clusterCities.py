...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for mine in self.points:
        	for theirs in other.points:
        		distance = mine.distance(theirs)
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        newCPoints = c1.points + c2.points
        newCluster = Cluster(newCPoints, self.pointType)
        self.add(newCluster)
    ...
    def findClosest(self, linkage):
        ...
        number = self.numClusters()
        tuples = []

        for i in range(number - 1):
        	for j in range(i + 1, number):
        		distance = linkage(self.members[i], self.members[j])
        		tuples.append((distance, self.members[i], self.members[j]))
...
def test():
    ...
    hCluster(points, Cluster.singleLinkageDist, 5, False)
	# points = buildCityPoints('test', False)
